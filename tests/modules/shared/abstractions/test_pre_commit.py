...
def settings_dict() -> dict:
    return settings.PreCommitSettings(
        repos=[
...
def mock_pre_commit(
    ...
    return pre_commit.PreCommitAbstraction(
        command_timeout_seconds=300,
    )


def test_that_pre_commit_executes_hooks_successfully(
    mock_pre_commit: pre_commit.PreCommitAbstraction,
    mock_subprocess: MagicMock,
):
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.execute_hooks(test_folder_path)
...
def test_that_pre_commit_executes_hooks_successfully_including_all_files(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.execute_hooks(test_folder_path, all_files=True)
...
def test_that_pre_commit_executes_a_single_hook_if_specified(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    mock_pre_commit.execute_hooks(test_folder_path, hook_id="detect-secrets")
...
def test_that_pre_commit_executes_hooks_on_specified_files(
    mock_pre_commit: pre_commit.PreCommitAbstraction, mock_subprocess: MagicMock
):

    files = ["test_file.py", "test-file.js"]
    mock_subprocess.return_value = CompletedProcess(args=[], returncode=0)
    mock_pre_commit.execute_hooks(
        test_folder_path,
...
def test_that_pre_commit_autoupdate_hooks_executes_successfully(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.autoupdate_hooks(test_folder_path)
...
def test_that_pre_commit_autoupdate_hooks_executes_successfully_with_bleeding_edge(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.autoupdate_hooks(
...
def test_that_pre_commit_autoupdate_hooks_executes_successfully_with_freeze(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.autoupdate_hooks(test_folder_path, freeze=True)
...
def test_that_pre_commit_autoupdate_hooks_executes_successfully_with_repos(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.autoupdate_hooks(
        test_folder_path, repos=test_repos
    )
...
def test_that_pre_commit_autoupdate_hooks_ignores_repos_when_repos_is_a_dict(
    ...
    test_repos = {}
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.autoupdate_hooks(test_folder_path, repos=test_repos)  # type: ignore
...
def test_that_pre_commit_update_executes_successfully(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.update(test_folder_path)
...
def test_that_pre_commit_remove_unused_hookss_executes_successfully(
    ...
    mock_subprocess.run.return_value = CompletedProcess(args=[], returncode=0)
    execute_result = mock_pre_commit.remove_unused_hooks(test_folder_path)
...
def test_that_pre_commit_install_creates_pre_commit_hook_for_secureli(
    ...
    with (
        ...
        result = mock_pre_commit.install(test_folder_path)

        assert result == pre_commit.InstallResult(
...
def test_pre_commit_config_file_is_deserialized_correctly(
    ...
    with um.patch("builtins.open", um.mock_open()) as mock_open:
        ...
        pre_commit_config = mock_pre_commit.get_pre_commit_config(test_folder_path)
...
def test_check_for_hook_updates_infers_freeze_param_when_not_provided(
    ...
    with um.patch(
        ...
        pre_commit_config_repo = settings.PreCommitRepo(
            repo="http://example-repo.com/",
            rev=rev,
            hooks=[settings.PreCommitHook(id="hook-id")],
        )
        pre_commit_config = settings.PreCommitSettings(repos=[pre_commit_config_repo])
        ...
        mock_pre_commit.check_for_hook_updates(pre_commit_config)
...
def test_check_for_hook_updates_respects_freeze_param_when_false(
    ...
    with um.patch(
        ...
        mock_hook_repo_rev_info.return_value = rev_info_mock
        rev_info_mock.update.return_value = rev_info_mock  # Returning the same revision info on update means the hook will be considered up to date
        mock_pre_commit.check_for_hook_updates(pre_commit_config, freeze=False)
...
def test_check_for_hook_updates_respects_freeze_param_when_true(
    ...
    with um.patch(
        ...
        mock_hook_repo_rev_info.return_value = rev_info_mock
        rev_info_mock.update.return_value = rev_info_mock  # Returning the same revision info on update means the hook will be considered up to date
        mock_pre_commit.check_for_hook_updates(pre_commit_config, freeze=True)
...
def test_check_for_hook_updates_returns_repos_with_new_revs(
    ...
    with um.patch(
        ...
        old_rev = "tag1"
        repo_1_new_rev = "tag2"
        pre_commit_config = settings.PreCommitSettings(
            repos=[
                settings.PreCommitRepo(
                    repo=repo_url,
                    rev=old_rev,
        ...
        updated_repos = mock_pre_commit.check_for_hook_updates(pre_commit_config)
...
def test_pre_commit_config_exists(mock_pre_commit: pre_commit.PreCommitAbstraction):
    with um.patch.object(Path, "exists", return_value=True):
        pre_commit_exists = mock_pre_commit.pre_commit_config_exists(test_folder_path)
