...
from secureli.modules.language_analyzer import git_ignore
...
def mock_path(mocker: MockerFixture) -> MagicMock:
    mock_path_class = mocker.patch("secureli.modules.language_analyzer.git_ignore.Path")
...
@pytest.fixture
def git_ignore_fixture(mock_path: MagicMock) -> git_ignore.GitIgnoreService:
    git_ignore_fixture = git_ignore.GitIgnoreService()
    git_ignore_fixture.git_ignore_path = mock_path
    return git_ignore_fixture
...
def test_that_git_ignore_creates_file_if_missing(
    ...
    with um.patch.object(Path, "exists") as mock_exists:
        ...
        git_ignore_fixture.ignore_secureli_files()
        ...
        assert args[0].find(git_ignore_fixture.header) != -1
        assert args[0].find(git_ignore_fixture.footer) != -1
...
def test_that_ignore_ignore_finds_and_reads_file(
    ...
    ignored_patterns = git_ignore_fixture.ignored_file_patterns()
